...
int main ( int argc, char* argv[] )
{
	...
	struct stat sts ;
	int tube[2], red_pipe[2], blue_pipe[2], green_pipe[2] ;
	char ref[4096] ;
	char res[1024]  = "copie";
	...
	{		//fils lecteur?
		...
		if(!pid1){
			...
			sleep(2);
			if(execl("./recouvrant","recouvrant",chR,chR2,ch3,ch4,ch6,ref,res,"R",NULL) < 0)
				fprintf(stderr, "execl error red son\n");
			exit(-1);
		}
		else{
			...
			if(!pid2){
				...
				sprintf(chB2, "%d", blue_pipe[1]) ;
				sleep(2);
				if(execl("./recouvrant","recouvrant",chB,chB2,ch3,ch4,ch6,ref,res,"B",NULL) < 0)
					fprintf(stderr, "execl error blue son\n");
				exit(-1) ;
			}
			else{
				...
				if(!pid3){
					...
					sleep(2);
					if(execl("./recouvrant","recouvrant",chV,chV2,ch3,ch4,ch6,ref,res,"V",NULL) < 0)
						fprintf(stderr, "execl error green son\n");
					exit(-1);
				}
				else{
					...
					if(infoHeader.bits_per_pixel == 24)
					{
						...
						while(read(desc3, &buffer_tp, sizeof(char)) > 0){
							if( write(tube[1], &buffer_tp, sizeof(char)) == -1 || write(red_pipe[1], &buffer_tp, sizeof(char)) == -1 || write(green_pipe[1], &buffer_tp, sizeof(char)) == -1 || write(blue_pipe[1], &buffer_tp, sizeof(char)) == -1){
